...
/*
 * 模式修正符号（单个字符）
 *  1、模式修正符号要写在定界符号外，放在右边；  "/go*gle/i"    i -- 不区分大小写
 *  2、模式修正符号，一个字符就是一个功能，可以组合使用。 "/go*gle/ieU"
 *
 * 作用：
 *  1、模式修正符号可以修正正则表达式的解释,或扩充了正则表达式的功能
 *
 *  i -- 模式中的字母会进行大小写不敏感匹配(默认是区分大小的)。
 *  m -- 修正正则表达式可以视为多行，在使用 ^ 或 $ 这两个符号时，每一行满足都可以（默认视为一行处理）。
 *  s -- 修正正则表达式中的，可以匹配换行符号(默认 . 不能匹配回车符号)
 *  x -- 修正正则表达式可以省略空白
 *  U -- (.*,.+)正则表达式比较贪婪; .*?  .+?
 */
$s2 = "This is google test<br>";
$reg5 = "/TEST/i";
echo preg($reg5, $s2) . '<br>';
$s3 = "this <b>is</b> xdd rrt <b>abc</b>  WebServer te
st hello world
yanzi";
$reg6 = "/^hel/mi";
echo preg($reg6, $s3) . "<br>";
$reg7 = "/te.+st/s";
echo preg($reg7, $s3) . '<br>';
$reg8 = "/web server/ix";
echo preg($reg8, $s3) . '<br>';
$reg9 = '/\<b\>(.*)\<\/b\>/isU';
echo preg($reg9, $s3) . '<br>';
